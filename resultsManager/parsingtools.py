...
class WcetResParser(LineParser):
    def __init__(self):
        super(WcetResParser, self).__init__(re.compile(r'WCET = (\d+)'))
...
class BBInfoParser(LineParser):
    def __init__(self):
        a = (
            r'(\S+_\S+)\|(\d+) evts\|(\d+) INBLOCK evts\|(\d+) XG Nodes\|(\d+) final Nodes\|(\d+) final Leafs\|(\d+) Manager Nodes\|analysis time = (\d+)')
...
class DcacheAnalysisRes(FileParser):
    def __init__(self):
        super(DcacheAnalysisRes, self).__init__(re.compile(
            r'always-hit:\s*(\d+)\s*\((\d+)\..*\).*\s*first-hit:\s*(\d)+\s*\((\d+)\..*\).*\s*first-miss:\s*(\d+)\s*\((\d+)\..*\).*\s*always-miss:\s*(\d+)\s*\((\d+)\..*\).*\s*not-classified:\s*(\d+)\s*\((\d+)\..*\).*\s*total:\s*(\d+)'))
